function rej_fif2blinkrej(expt,subjID,paradigmName, do_plot)
%------------------------
%
...
if exist(sub_thr, 'file')
    ...
    rej_path = '/Users/Shared/MNE_scripts/function_inputs/rej_thr.txt';
end
S = importdata(rej_path);

% find each rej thr
[~, i] = ismember(S.textdata, 'blink');
blink_rej = S.data(i == 1)
...
for chan = chan_c
    chan_str = chan{1};
        switch chan_str
            case 'blink'
                chans = [1 2 3 22 23 24 25 42 43 44 45 60 61 62 63 64 83 84 85 101];
                meth = 'win';
                thr = blink_rej;
                f = @window;
            otherwise
